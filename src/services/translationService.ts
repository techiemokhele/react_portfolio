...
export const isEnglish = (text: string): boolean => {
  ...
  const matches = words.filter((w) => COMMON_ENGLISH.includes(w.replace(/[.,!?;:()]/g, ""))).length;
  return matches / words.length > 0.1;
};

const LANGUAGE_PATTERNS: Record<string, string[]> = {
  es: ["el","la","los","las","de","que","es","en","un","una","por","para","con","del"],
  fr: ["le","la","les","de","un","une","et","est","dans","pour","qui","sur","avec","pas"],
  de: ["der","die","das","und","ist","in","den","von","zu","mit","ein","eine","nicht"],
  pt: ["de","que","o","a","os","as","um","uma","para","com","não","em","por","do","da"],
  it: ["il","la","di","che","è","un","una","per","con","non","in","da","del","della"],
...
export const translateToEnglish = async (text: string, sourceLang = ""): Promise<string> => {
  ...
  try {
    ...
    if (data.responseStatus === 200 && data.responseData?.translatedText) {
      ...
      return text.length > 500 ? translated + " " + text.substring(500) : translated;
    }
    return text;
  } catch {
    return text;
...
const translateFullDescription = async (html: string, sourceLang = ""): Promise<string> => {
  if (!html) return html;
  ...
  for (const seg of segments) {
    if (seg.startsWith("<") || !seg.trim() || seg.trim().length <= 3 || isEnglish(seg.trim())) {
      out.push(seg);
    } else {
      try {
        const translated = await translateToEnglish(seg.trim(), sourceLang);
        out.push(seg.replace(seg.trim(), translated));
        if (seg.trim().length > 20) await new Promise((r) => setTimeout(r, 400));
      } catch {
        out.push(seg);
      }
    }
...
export const translateArticle = async (article: BlogPost): Promise<BlogPost> => {
  ...
  try {
    const detectedLang = detectLanguage(article.title || article.excerpt || "") || undefined;
    ...
    return { ...article, originalTitle: article.title, originalExcerpt: article.excerpt, originalDescription: article.fullDescription, isTranslated: false };
  }
};
